...
class  Korangi extends Component {
    constructor() {
        super();
        this.state = {
            date:'',
    ...
    componentWillMount() {
        ...
        firebase.database().ref('parking/korangi').on(
            'value', snap => {
...
componentWillMount() {
    ...
    handleOpen = (index) => {
        ...
        firebase.database().ref(`parking/korangi/slot${index}`).on(
            'value', snap => {
      ...
      dialogSubmitButton = () => {
        ...
        firebase.database().ref(`parking/korangi/slot${this.state.slotNumber}`).set(this.state.TimeSaveArray)
